...
export function ContactsPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setSelectedId(saved.id); setCreating(false); setFeedback("Alterações salvas.");
    ...
    <div className="contacts-layout">
      <section className="contacts-list-panel">
        ...
        <div className="contacts-list">
          ...
          {filtered.map((contact) => <button className={contact.id === selectedId && !creating ? "contact-row active" : "contact-row"} key={contact.id} onClick={() => { setCreating(false); setSelectedId(contact.id); }} type="button"><span className="conversation-avatar"><UserRoundCog size={16} /></span><span><strong>{contact.name}</strong><small>{contact.phone}</small><span className="tag-row">{userContactTags(contact.tags).slice(0, 2).map((tag) => <i key={tag}>{tag}</i>)}</span></span><Badge variant={contact.kind === "lead" ? "accent" : "success"}>{kindLabels[contact.kind]}</Badge></button>)}
        </div>
      </section>
      <aside className="contact-detail-panel">
        ...
        <div className="contact-info-grid"><div><Phone size={15}/><span>{form.phone || "—"}</span></div><div><Mail size={15}/><span>{form.email || "—"}</span></div><div><Tags size={15}/><span>{userContactTags(form.tags).join(", ") || "—"}</span></div></div>
