...
import { Bot, Building2, Coins, LogOut, MessageSquare, Plus, ShieldCheck, Users } from "lucide-react";
...
type Dashboard = {
  ...
  estimated_ai_cost:string; credits_outstanding:number;
};
type Tenant = {
  id:string; name:string; slug:string; status:string; created_at:string; users:number;
  conversations:number; leads:number; properties:number; contacts:number; ai_calls:number;
  estimated_ai_cost:string; credit_balance:number; credit_enforcement:"meter_only"|"enforce";
  unlimited_messages:boolean; integrations:Record<string,string>;
};
...
export function PlatformApp() {
  ...
  return <main className="platform-page page-stack">
    ...
    <section className="metrics-grid">
      ...
      <MetricCard icon={Bot} label="Chamadas de IA" value={dashboard?.ai_calls ?? 0} detail={`Custo OpenAI: US$ ${dashboard?.estimated_ai_cost ?? "0"}`} />
      <MetricCard icon={Coins} label="Créditos em carteira" value={dashboard?.credits_outstanding ?? 0} detail="Saldo total dos clientes" />
    </section>
...
function TenantDetail({tenant,token,onToggle,onChanged}:{tenant:Tenant;token:string;onToggle:()=>void;onChanged:()=>void}) {
  ...
  async function grant(event:FormEvent){
    ...
    try {
      await request(`/platform/tenants/${tenant.id}/credits/grants`,{method:"POST",body:JSON.stringify({credits:Number(credits),description,idempotency_key:crypto.randomUUID()})},token);
      setFeedback("Créditos adicionados.");onChanged();
    } catch(reason){setFeedback(readError(reason));}
  }
  async function savePolicy(){
    try {
      await request(`/platform/tenants/${tenant.id}/credits/settings`,{method:"PATCH",body:JSON.stringify({enforcement_mode:tenant.credit_enforcement==="enforce"?"meter_only":"enforce",unlimited_messages:tenant.unlimited_messages})},token);
      onChanged();
    } catch(reason){setFeedback(readError(reason));}
  }
  async function toggleUnlimited(){
    try {
      await request(`/platform/tenants/${tenant.id}/credits/settings`,{method:"PATCH",body:JSON.stringify({enforcement_mode:tenant.credit_enforcement,unlimited_messages:!tenant.unlimited_messages})},token);
      onChanged();
    } catch(reason){setFeedback(readError(reason));}
  }
  return <div className="page-stack"><div><span className="eyebrow">Cliente</span><h2>{tenant.name}</h2><p>{tenant.slug} · criado em {new Date(tenant.created_at).toLocaleDateString("pt-BR")}</p></div><div className="contact-info-grid"><div><Users size={15}/><span>{tenant.users} usuários</span></div><div><span>{tenant.contacts} contatos</span></div><div><span>{tenant.properties} imóveis</span></div><div><span>{tenant.conversations} conversas</span></div><div><span>{tenant.leads} leads</span></div><div><span>{tenant.ai_calls} chamadas IA · US$ {tenant.estimated_ai_cost}</span></div></div><div className="settings-subsection"><h3>Créditos</h3><div className="contact-info-grid"><div><Coins size={15}/><span>{tenant.credit_balance.toLocaleString("pt-BR")} disponíveis</span></div><div><span>{tenant.credit_enforcement==="enforce"?"Bloqueio sem saldo":"Somente medição"}</span></div><div><span>{tenant.unlimited_messages?"Mensagens ilimitadas":"Mensagens consomem créditos"}</span></div></div><form className="form-grid" onSubmit={grant}><Field label="Adicionar créditos" type="number" value={credits} onChange={setCredits}/><Field label="Motivo" value={description} onChange={setDescription}/><button type="submit">Lançar créditos</button></form><div className="settings-actions"><button className="button-outline" onClick={()=>void savePolicy()} type="button">{tenant.credit_enforcement==="enforce"?"Desativar bloqueio":"Bloquear ao zerar"}</button><button className="button-outline" onClick={()=>void toggleUnlimited()} type="button">{tenant.unlimited_messages?"Cobrar mensagens":"Mensagens ilimitadas"}</button></div>{feedback?<p>{feedback}</p>:null}</div><div><h3>Integrações</h3><p>{Object.entries(tenant.integrations).map(([name,status])=>`${name}: ${status}`).join(" · ")||"Nenhuma integração configurada"}</p></div><button className={tenant.status === "active" ? "button-danger" : ""} onClick={onToggle} type="button">{tenant.status === "active" ? "Suspender cliente" : "Reativar cliente"}</button></div>;
}
